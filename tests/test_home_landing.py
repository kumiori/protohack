...
def test_sidebar_is_local_or_explicitly_enabled_and_hidden_by_default_in_production() -> None:
    app_source = APP_ENTRY.read_text(encoding="utf-8")

    assert 'initial_sidebar_state="expanded" if DEVELOPER_SIDEBAR else "collapsed"' in app_source
    assert "if not DEVELOPER_SIDEBAR:" in app_source
    assert "developer_sidebar_enabled()" in app_source
