...
def apply_theme() -> None:
    st.markdown(
        ...
        :root { --ink:#12211b; --paper:#f3f0e8; --acid:#d7ff48; --mint:#b9ead7; }
        *, *::before, *::after { box-sizing:border-box; }
        html, body, .stApp { overflow-x:hidden; }
        .stApp { background: var(--paper); color: var(--ink); }
        .block-container { width:100%; max-width:1280px; }
        html, body, [class*="css"] { font-family: "Manrope", sans-serif; }
        h1, h2, h3 { letter-spacing:-0.045em !important; color:var(--ink) !important; }
        h1 { font-size:clamp(2.7rem, 7vw, 5.9rem) !important; line-height:.92 !important; }
        code, .mono { font-family:"DM Mono", monospace; }
        [data-testid="stHeader"] { background:transparent; }
        [data-testid="stSidebar"] { background:#12211b; }
        [data-testid="stSidebar"] * { color:#f3f0e8; }
        .eyebrow { font-family:"DM Mono", monospace; text-transform:uppercase; letter-spacing:.12em; font-size:.76rem; }
        .hero { padding:3.2rem 0 1.5rem; max-width:1000px; }
        .hero p { max-width:760px; font-size:1.24rem; line-height:1.5; overflow-wrap:anywhere; }
        ...
        .pilot-badge { display:inline-flex; align-items:center; gap:.35rem; border:1px solid rgba(18,33,27,.45); border-radius:999px; padding:.25rem .58rem; font-family:"DM Mono",monospace; font-size:.68rem; letter-spacing:.08em; text-transform:uppercase; }
        ...
        .compact-masthead strong { font-size:1rem; letter-spacing:-.02em; }
        ...
        .review-action { font-size:clamp(2.3rem,7vw,5.5rem); font-weight:800; letter-spacing:-.055em; line-height:.92; text-transform:uppercase; max-width:1050px; margin:.5rem 0 1.35rem; overflow-wrap:anywhere; }
        .review-description { max-width:760px; color:rgba(18,33,27,.7); font-size:1rem; line-height:1.5; }
        .review-because { margin-top:2rem; }
        .review-rationale { font-size:clamp(1.25rem,2.5vw,1.9rem); line-height:1.4; max-width:900px; margin-top:.4rem; }
        ...
        .reward h2 { font-size:clamp(2rem,5vw,4.2rem) !important; margin:.45rem auto; max-width:820px; }
        ...
        div.stButton > button, div.stFormSubmitButton > button { border:2px solid var(--ink); border-radius:999px; background:var(--acid); color:var(--ink); font-weight:800; min-height:3rem; box-shadow:4px 4px 0 var(--ink); }
        ...
        .footer { margin-top:4rem; padding:1.2rem 0 2rem; border-top:1px solid rgba(18,33,27,.35); font-family:"DM Mono",monospace; font-size:.78rem; }
        @media (max-width: 640px) {
          .block-container { padding-left:1rem; padding-right:1rem; }
          .hero { padding-top:1rem; }
          .scenario { box-shadow:5px 5px 0 var(--ink); }
          h1 { font-size:clamp(2.65rem, 13vw, 4rem) !important; overflow-wrap:anywhere; }
...
def header(
    ...
    *,
    ...
    if show_storage_warning and repository_mode() == "demo":
        st.info("Preview storage is active. Records remain in this running app only; Notion is not being written.", icon="🧪")
...
def action_counts(profiles: list[dict[str, Any]]) -> Counter[str]:
    return Counter(str(profile.get("action_label") or profile.get("action_id")) for profile in profiles)
...
def strategic_graph_dot(profiles: list[dict[str, Any]]) -> str:
    ...
    for index, profile in enumerate(profiles):
        ...
        for tag in profile.get("semantic_tags") or []:
            safe_id = "tag_" + "".join(character if character.isalnum() else "_" for character in str(tag))
